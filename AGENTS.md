...
- AI analysis is `lib/ai.ts` (server-only). Env vars: `GROQ_API_KEY` (required), `AI_BASE_URL` (default Groq), `AI_MODEL` (default `openai/gpt-oss-120b`). Provider is swappable via these env vars. NOTE: the model list must be checked via Groq's `/models` endpoint — common model IDs like `llama-3.3-70b-versatile` may not exist on the account. `transcribeAudio` (also in `lib/ai.ts`) POSTs multipart to `{baseUrl}/audio/transcriptions` with `whisper-large-v3-turbo` (Groq) / `whisper-1` (OpenAI base), throws `AiQuotaError` on HTTP 429.
...
- AI key fallback: `getUserAiKey` lives in `lib/ai-quota.ts` (server-only, shared by `lib/actions.ts` and route handlers). Shared `GROQ_API_KEY` is tried first; on `AiQuotaError` (429) the user's stored AI key (`ai_keys` table, `decrypt_ai_key` RPC with `JIRA_TOKEN_KEY`) is used.
- Transcript capture: `components/AddMeetingForm.tsx` keeps a paste textarea (the single source for Save/Analyze) plus two fill helpers — a `.txt/.md/.vtt/.srt` file import parsed client-side (timestamps stripped), and a recording upload (≤25 MB, audio formats) POSTed to `app/api/transcribe/route.ts` (a route handler — route-handler `formData()` is not bound by the 1 MB server-action body limit, so no `next.config.ts` change is needed). The handler auths via `createClient()` + `auth.getUser()`, validates type/size, transcribes with the shared key then the user's own key, and returns `{ text }` / `{ error }`.
- Review workflow: `/feedback/[id]` shows an editable form (`components/FeedbackReviewForm.tsx`); `updateFeedback` in `lib/actions.ts` reads a submit button named `action` with values `save`/`generate`/`jira` plus a `status` field (`pending`/`approved`/`rejected`/`duplicate`) from the Review dropdown. Canonical `review_status` values are `pending`/`approved`/`rejected`/`duplicate` (badges + Dashboard counts depend on these). The ticket-builder section (Jira workspace, `ticket_type` select, `ticket_context` additional instructions, AI draft, Create) renders client-side only when the selected status is `approved` and no ticket exists. The `jira` action saves edits, verifies status is `approved` and no ticket exists, reads `connection_id` from the form, decrypts that connection's token, calls `createJiraTicket`, and inserts into `jira_tickets`. The form's `ticket_type` (defaulted to `jiraIssueTypeFor(feedback.type, connection.issue_type)` server-side as `ReviewItem.suggestedIssueType`) overrides the auto-mapped issue type when creating the ticket.
...
- Ticket syncing: `syncJiraTicket` in `lib/jira.ts` fetches current status/assignee/sprint from Jira (sprint field is discovered via `/rest/api/3/field` since it is a per-instance custom field). The `syncJiraTicketAction` server action (`lib/actions.ts`) decrypts the connection token, syncs, and updates `jira_tickets.status/sprint/assignee`. Manual triggers only: `/jira` renders `components/JiraTicketsTable.tsx` (client) with "Sync all from Jira" + per-row Sync buttons; there is no scheduled or automatic sync.
